...
def make_vip_unreachable(duthost, vip_addr):
    duthost.shell('sudo iptables -A INPUT -s {} -j DROP'.format(vip_addr))
    duthost.shell('sudo iptables -A OUTPUT -d {} -j DROP'.format(vip_addr))

def make_vip_reachable(duthost, vip_addr):
    duthost.shell('sudo iptables -D INPUT -s {} -j DROP'.format(vip_addr))
...
def check_connected(duthost, expected_status):
    ...
    for line in kube_server_status:
        if line.startswith("KUBERNETES_MASTER SERVER connected"):
            assert line.endswith(expected_status)
            break
...
def test_join_available_master(precheck_k8s_vms, duthost, k8shosts):
    ...
    duthost.shell('sudo config kube server ip {}'.format(vip_addr))
