...
def patch_cluster_info_to_ip(vmhost, creds):
    ...
    We patch it to use https://<vmhost.mgmt_ip>:6443 and re-sign so discovery succeeds.
    """
    http_proxy = creds.get("proxy_env", {}).get("http_proxy", "")
...
def update_kubelet_config(vmhost, creds):
    ...
    # minimal safety: include vmhost IP to avoid proxying apiserver requests
    no_proxy_param = f"{NO_PROXY},{vmhost.mgmt_ip}"
    proxy_param = f"{http_proxy_param} {https_proxy_param} {no_proxy_param}".strip()

    tmp_kubelet_config = "/tmp/kubelet-config.yaml"
    # minimal fix: pass kubectl flags after "--" so they go to kubectl, not minikube
...
def setup_and_teardown(duthost, vmhost, creds):
    ...
    # Keep /etc/hosts entry (in case DNS is needed)
